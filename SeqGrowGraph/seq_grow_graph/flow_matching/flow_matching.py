...
from ..lane_diffusion.swin_unet import SwinTransformerUNet
...
class SwinVectorField(nn.Module):
    """Wrap SwinTransformerUNet to serve as a vector field."""

    def __init__(self, bev_channels: int, swin_cfg: Optional[dict] = None):
        super().__init__()
        swin_cfg = swin_cfg.copy() if swin_cfg is not None else {}
        default_cfg = dict(
        ...
        default_cfg.update(swin_cfg)
        self.denoiser = SwinTransformerUNet(**default_cfg)

    def forward(self, x_t: torch.Tensor, x0: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        # Swin UNet expects timestep in [0, 1]
        t = torch.clamp(t, 0.0, 1.0)
        return self.denoiser(x_t, x0, t)
...
class FlowMatchingBEV(nn.Module):
    ...
    def __init__(
        ...
        bev_h: int = 128,
        bev_w: int = 192,
        lpim_config: Optional[dict] = None,
        ...
        elif vector_type == "swin":
            swin_cfg = flow_config.get("swin_cfg", None)
            if swin_cfg is None:
                swin_cfg = {}
            swin_cfg.setdefault("window_size", 8)
            self.vector_field = SwinVectorField(
                bev_channels=bev_channels,
                swin_cfg=swin_cfg,
            )
        else:
    ...
    def sample(self, x0: torch.Tensor, num_steps: Optional[int] = None, noise_std: float = 0.0):
